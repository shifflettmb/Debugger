...
public class Creature implements GLEventListener, KeyListener, MouseListener, MouseMotionListener
{
	private Butterfly[] Creature = new Butterfly[1];
	private centipede[] centi = new centipede[1]; 
	...
	private float viewangle=0f, stepsize=1f, pan=2f;
	public float eyex=-1f, eyey=4f, eyez=-1f; 
	...
	public void init(GLAutoDrawable drawable) 
	{
		...
		Creature[i] = new Butterfly(gl, canvas, 2f, 12f);
		for(int i=0; i<Muffet.length; i++)
			Muffet[i]=new Spider(gl, canvas, 0f,0f); 
		field = new Grass(gl,0f,0f,0f,20f); 
		for(int i=0; i<centi.length; i++)
			centi[i] = new centipede(gl, canvas, 2f, 2f);
	}
	...
	public static void main(String args[])
	{
		...
		Creature renderer = new Creature();
